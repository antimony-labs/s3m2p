use dna::{
    ...
    compute_flocking_forces, process_predation, process_scavenging,
};
use glam::Vec2;
...
fn test_mass_extinction_mechanics() {
    ...
    assert!(arena.alive_count < 150, "Too many survivors: {}", arena.alive_count);
    assert!(arena.alive_count > 10, "Too few survivors (founders failed?): {}", arena.alive_count);
    
    // 4. Verify founders were seeded
    // Founders are seeded with random velocities, so they shouldn't be all zero if we had them
    let moving_count = arena.iter_alive()
...
fn test_diversity_metric() {
    ...
    assert!(div_high > div_low, "Mixed population should have higher diversity than monoculture. Low: {}, High: {}", div_low, div_high);
    assert!(div_high > 0.5, "Balanced population should have good diversity score");
}
...
fn test_simulation_stability_long_run() {
    ...
    for _frame in 0..1000 {
        ...
        for idx in arena.iter_alive() {
            ...
            assert!(!pos.x.is_nan() && !pos.y.is_nan(), "NaN position detected at frame {}", _frame);
            
            let vel = arena.velocities[idx];
            assert!(!vel.x.is_nan() && !vel.y.is_nan(), "NaN velocity detected at frame {}", _frame);
        }
    }
}
...
fn test_predator_prey_dynamics() {
    ...
    let scav_idx = arena.spawn(Vec2::new(12.0, 10.0), Vec2::ZERO, scav_genes).index();
    
    let carn_idx = arena.spawn(Vec2::new(10.0, 10.0), Vec2::ZERO, carn_genes).index();
    ...
    assert!(arena.energy[scav_idx] > 100.0, "Scavenger should gain energy from corpse. Current: {}", arena.energy[scav_idx]);
}
